...
def exportCSV(list1, list2, list3, list4):
        df = pd.DataFrame(columns=['TIMESTAMP', 'AMOUNT', 'PRICE', 'SIDE','ORDERID','LIST', 'OFFLOAD', 'ORDERSRCID', 'S_PRICE', 'B_PRICE', 'TRANSFERT', 'BTC_TRANSFERED', 'ETH_TRANSFERED'])
        i = 0
        for t in list1:
            df.loc[i] = [t['timestamp']] + [t['amount']] + [t['price']] + [t['side']] + [t['id']] + ["WAIT_SELL"] + ["False"] + ["0"] + [t['s_price']] + ["0"] + [t['transfert']] + [t['btc_transfered']] + [t['eth_transfered']] 
            i += 1
        for t in list2:
            df.loc[i] = [t['timestamp']] + [t['amount']] + [t['price']] + [t['side']] + [t['id']] + ["WAIT_BUY"] + ["False"] + ["0"] + ["0"] + [t['b_price']] +  [t['transfert']] + [t['btc_transfered']] + [t['eth_transfered']] 
            i += 1
        for t in list3:
            df.loc[i] = [t['timestamp']] + [t['amount']] + [t['price']] + [t['side']] + [t['id']] + ["SELL_PROFIT"] + [t['OFFLOAD']] + [t['src_id']] + ["0"] + ["0"] + ["False"] + ["0"] + ["0"]
            i += 1
        for t in list4:
            df.loc[i] = [t['timestamp']] + [t['amount']] + [t['price']] + [t['side']] + [t['id']] + ["BUY_PROFIT"] + [t['OFFLOAD']] + [t['src_id']] + ["0"] + ["0"] + ["False"] + ["0"] + ["0"]
            i += 1    
        df = df[['TIMESTAMP', 'AMOUNT', 'PRICE', 'SIDE','ORDERID','LIST', 'OFFLOAD','ORDERSRCID','S_PRICE', 'B_PRICE','TRANSFERT', 'BTC_TRANSFERED', 'ETH_TRANSFERED']]
        ...
        for sell_order_profit in list_sell_profit:
            ...
            for o in list_wait_buy:
                if o['id'] == sell_order_profit['src_id']:
                    if o['transfert'] == True:
                        transfered = True
            if ((p <= (sell_price + inload_step)) and sell_order_profit['OFFLOAD'] == True and transfered == False):
                try:
        ...
        for buy_order_profit in list_buy_profit:
            ...
            for o in list_wait_sell:
                if o['id'] == buy_order_profit['src_id']:
                    if o['transfert'] == True:
                        transfered = True
            if ((D(str(buy_order_profit['price'])) >= buy_price - inload_step) and buy_order_profit['OFFLOAD'] == True and transfered == False):
                try:
...
while True:
    ...
    for s in list_wait_sell:
        if (s['price'] + (0.01 * s['price']) > s_price and s['transfert'] == True):
            ...
            try:
                ...
                #client.make_subaccount_universal_transfer(["algolongeth_virtual@rr5c6nswnoemail.com","algoshorteth_virtual@7y44jnu5noemail.com","SPOT","SPOT","BTC",t_amount])
                #client.make_subaccount_universal_transfer(params)
    ...
    for b in list_wait_buy:
        if (b['price'] - (0.01 * b['price']) < b_price and b['transfert'] == True):
            t_amount = s['btc_transfered']
            try:
                params={
                ...
                #client.make_subaccount_universal_transfer(["algoshorteth_virtual@7y44jnu5noemail.com","algolongeth_virtual@rr5c6nswnoemail.com","SPOT","SPOT","ETH",t_amount])
                #client.make_subaccount_universal_transfer(params)
                tr= binance.sapiPostSubAccountUniversalTransfer(params)
            except Exception as e:
    ...
    if (Trend == 1):
        if b_price in long_set :
            ...
            try:
                b_order = binance_long.create_market_order('ETH/BTC', 'buy', amount / b_price)
    ...
    if (Trend == -1):
        if s_price in short_set :
            ...
            try:
                s_order = binance_short.create_market_order('ETH/BTC', 'sell', amount / s_price)
    ...
    if (w_long['BTC']['total'] < amount and Trend == 1):
        t_amount = D("0")
        for s in list_wait_sell:
            if (s['price'] + (0.02 * s['price']) < s_price and s['transfert'] == False):
                ## transfert
                ...
                try:
                    ...
                    #client.make_subaccount_universal_transfer(["algoshorteth_virtual@7y44jnu5noemail.com","algolongeth_virtual@rr5c6nswnoemail.com","SPOT","SPOT","BTC",t_amount])
                    #client.make_subaccount_universal_transfer()
                    #tr = binance.transfer("BTC",t_amount,"algoshorteth_virtual@7y44jnu5noemail.com","algolongeth_virtual@rr5c6nswnoemail.com", {})
                    #binance_short.transfer("BTC",t_amount,"algoshorteth_virtual@7y44jnu5noemail.com","algolongeth_virtual@rr5c6nswnoemail.com")
                    tr= binance.sapiPostSubAccountUniversalTransfer(params)
                except Exception as e:
    ...
    if (w_short['ETH']['total'] < (amount / b_price) and Trend == -1):
        t_amount = D("0")
        for b in list_wait_buy:
            if (b['price'] - (0.02 * b['price']) > b_price and b['transfert'] == False):
                ## transfert
                ...
                try:
                    ...
                    #client.make_subaccount_universal_transfer(["algolongeth_virtual@rr5c6nswnoemail.com","algoshorteth_virtual@7y44jnu5noemail.com","SPOT","SPOT","ETH",t_amount])
                    #client.make_subaccount_universal_transfer(params)                
                    #tr = binance.transfer("ETH",t_amount,"algolongeth_virtual@rr5c6nswnoemail.com","algoshorteth_virtual@7y44jnu5noemail.com", {})
                    tr= binance.sapiPostSubAccountUniversalTransfer(params)
                    #binance_long.transfer("ETH",t_amount,"algolongeth_virtual@rr5c6nswnoemail.com","algoshorteth_virtual@7y44jnu5noemail.com")
                except Exception as e:
    ...
    print ("Transfert BTC form short to long: ",transfert_btc_short_to_long)
    print ("Transfert ETH form long to short: ",transfert_eth_long_to_short)
