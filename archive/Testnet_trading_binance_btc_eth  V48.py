...
while True:
    ...
    if (w_long['BTC']['total'] < amount and Trend == 1):
        ...
        for s in list_wait_sell:
            if (s['price'] + (0.01 * s['price']) < s_price and s['transfert'] == False and w_short['BTC']['total'] > (min_wallet_amount * amount) ):
                ## transfert
    ...
    if (w_short['ETH']['total'] < (amount / b_price) and Trend == -1 and w_long['ETH']['total'] > ((min_wallet_amount * amount)/b_price)):
        t_amount = D("0")
