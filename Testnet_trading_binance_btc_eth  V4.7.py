...
transfert_eth_long_to_short = 0
transfert_btc_short_to_long = 0
...
def exportCSV(list1, list2, list3, list4):
        df = pd.DataFrame(columns=['TIMESTAMP', 'AMOUNT', 'PRICE', 'SIDE','ORDERID','LIST', 'OFFLOAD', 'ORDERSRCID', 'S_PRICE', 'B_PRICE'])
        i = 0
        for t in list1:
            df.loc[i] = [t['timestamp']] + [t['amount']] + [t['price']] + [t['side']] + [t['id']] + ["WAIT_SELL"] + ["False"] + ["0"] + [t['s_price']] + ["0"]
            i += 1
        for t in list2:
            df.loc[i] = [t['timestamp']] + [t['amount']] + [t['price']] + [t['side']] + [t['id']] + ["WAIT_BUY"] + ["False"] + ["0"] + ["0"] + [t['b_price']]
            i += 1
        for t in list3:
            df.loc[i] = [t['timestamp']] + [t['amount']] + [t['price']] + [t['side']] + [t['id']] + ["SELL_PROFIT"] + [t['OFFLOAD']] + [t['src_id']] + ["0"] + ["0"]
            i += 1
        for t in list4:
            df.loc[i] = [t['timestamp']] + [t['amount']] + [t['price']] + [t['side']] + [t['id']] + ["BUY_PROFIT"] + [t['OFFLOAD']] + [t['src_id']] + ["0"] + ["0"]
            i += 1    
        df = df[['TIMESTAMP', 'AMOUNT', 'PRICE', 'SIDE','ORDERID','LIST', 'OFFLOAD','ORDERSRCID','S_PRICE', 'B_PRICE']]
        ...
        for buy_order_profit in list_buy_profit:
            if ((D(str(buy_order_profit['price'])) >= buy_price - inload_step) and buy_order_profit['OFFLOAD'] == True):
                try:
                    ...
                    #    buy_order_amount = D(str(buy_order_profit['amount']))
                    #    buy_order_cost = buy_order_amount * buy_order_price
                    #    a =  (buy_order_cost/buy_price)
                    #    buy_order_price = buy_price
                    #    buy_order_price = buy_order_price + D(str("0.00001"))
                    #    tag1 = True
                    #else:
                    #    a =  buy_order_profit['amount']
                    #o = binance.create_order('ETH/BTC', 'limit', 'buy', buy_order_profit['amount'], buy_order_profit['price'])
                    #o = binance_short.create_order('ETH/BTC', 'limit', 'buy', (amount / buy_price) + amount , buy_order_profit['price'])
                    #a1 = D(str(a))
                    #precision = D('0.00000001')
                    #a2 = a1.quantize(precision,ROUND_HALF_UP)
                    #am = float(a2)
                    #precision = D('0.00001')
                    #p = buy_order_price.quantize(precision,ROUND_HALF_DOWN)
                    #price = float(p)
                    
                    if  buy_order_price  > buy_price:
                        o = binance_short.create_market_order('ETH/BTC', 'buy', buy_order_profit['amount'])
                        #o = binance_short.create_market_order('ETH/BTC', 'buy', amount/buy_price)
                    else:
...
while True:
    ...
    #except:
    #    continue
    #if (w_long['BTC']['total'] < amount):
    #    for s  in list_wait_sell:
    #        if (s['price'] + (s['price'] * gain) < s_price and s['transfered'] == "False"):
